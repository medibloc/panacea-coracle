...
func (svc *dataPoolService) handleSampleData(w http.ResponseWriter, r *http.Request) {
	...
	for i := uint64(1); i < uint64(10); i++ {
		data := []byte("{ " +
			"\"name\": \"This is a name\", " +
			"\"description\": \"pool - " + poolID + " | round : " + round + " | cert : " + strconv.FormatUint(i, 10) + "\"" +
		...
		filename := base64.StdEncoding.EncodeToString([]byte("data-" + poolID + "-" + round + "-" + strconv.FormatUint(i, 10)))
	...
	log.Infof("round %s data are generated", round)
